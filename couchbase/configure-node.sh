...
curl -v -X POST http://127.0.0.1:8091/pools/default -d memoryQuota=2000 -d indexMemoryQuota=512
...
#Create buckets
curl -v -X POST -u Administrator:password -d name=todo -d ramQuotaMB=200 -d authType=none  -d flushEnabled=1  -d proxyPort=11216 http://localhost:8091/pools/default/buckets
